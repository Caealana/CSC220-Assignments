...
var graphType = 0; //initial graph type is bar
...
Point.prototype.drawYearLabel = function(){ //creates label and adds it to html document
     //label for the bars
    //var yearLabel = document.createElement("div");
    //yearLabel.className = "barLabel";
    //yearLabel.style.position = "absolute";
    var yPosLabel = c.height - c.height/120;
    ...
    ctx.font = "15px Arial";
    ctx.fillText(this.year,this.xCord,yPosLabel);
...
function displayGraph(){
    ...
    if (graphType === 0){
        var barGraph = new Graph(DATA[dataIndex]);
        barGraph.updateTitle();
        barGraph.getMultValue();
        barGraph.createGraphBars();
        
        var lineGraph = new Graph(DATA[dataIndex]);
        lineGraph.updateTitle();
        lineGraph.getMultValue();
        lineGraph.createPoints();
        lineGraph.connectPoints();
    }
}
